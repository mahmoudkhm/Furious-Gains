...
public class ReinitialiserPwdSMS {
...
private static String code;
    ...
    public ReinitialiserPwdSMS(Connection cnx) {
    ...
    void Send(ActionEvent event) throws SQLException {
        ...
        String req = "SELECT user.*, codepromo.code FROM user JOIN codepromo ON user.id_code_promo = codepromo.id_code_promo  WHERE num_tel = ?";
        ...
        if (!adresse.getText().isEmpty()) {
           ...
           // mail.sendEmail(adresse.getText(),"Réinitialiser Votre mot de passe","Votre Code est :  " + nouveauMotDePasse + "\n S'il te plait ne passe pas 5 min De maintenant");
            ...
            try {
                ...
                alert.setTitle("Erreur de saisie: champ Numero ");
            ...
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Erreur de saisie: champ Numero est vide");
            alert.setHeaderText((String)null);
            alert.setContentText("Il est obligatoire de mettre le numero de telephone !");
            alert.showAndWait();
        }
        }
    ...
    void envoyerParSMS(ActionEvent event) {
        Parent root = null;
        try {
            root = FXMLLoader.load(getClass().getResource("/ReinitialiserPwd.fxml"));
